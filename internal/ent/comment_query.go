...
func (_q *CommentQuery) QueryVisit() *VisitQuery {
...
func (_q *CommentQuery) QueryAsset() *AssetQuery {
...
func (_q *CommentQuery) WithVisit(opts ...func(*VisitQuery)) *CommentQuery {
	query := (&VisitClient{config: _q.config}).Query()
...
func (_q *CommentQuery) WithAsset(opts ...func(*AssetQuery)) *CommentQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := _q.withVisit; query != nil {
		if err := _q.loadVisit(ctx, query, nodes, nil,
			func(n *Comment, e *Visit) { n.Edges.Visit = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAsset; query != nil {
		if err := _q.loadAsset(ctx, query, nodes, nil,
			func(n *Comment, e *Asset) { n.Edges.Asset = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadVisit(ctx context.Context, query *VisitQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Visit)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].VisitID == nil {
			continue
		}
		fk := *nodes[i].VisitID
...
func (_q *CommentQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Asset)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
...
func (_q *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withVisit != nil {
			_spec.Node.AddColumnOnce(comment.FieldVisitID)
		}
		if _q.withAsset != nil {
			_spec.Node.AddColumnOnce(comment.FieldAssetID)
		}
	}
