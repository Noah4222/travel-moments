...
func (tx *Tx) init() {
	...
	tx.AssetView = NewAssetViewClient(tx.config)
	tx.AuditLog = NewAuditLogClient(tx.config)
	tx.Collection = NewCollectionClient(tx.config)
	tx.CollectionAsset = NewCollectionAssetClient(tx.config)
	tx.Comment = NewCommentClient(tx.config)
	tx.ShareLink = NewShareLinkClient(tx.config)
	tx.ShareTrip = NewShareTripClient(tx.config)
	tx.Trip = NewTripClient(tx.config)
	tx.TripEditor = NewTripEditorClient(tx.config)
	tx.UploadGrant = NewUploadGrantClient(tx.config)
