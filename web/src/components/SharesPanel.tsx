...
function StatsModal({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  useEffect(() => {
    api.shareStats(id).then(setStats);
  }, [id]);
  return (
      ...
      {!stats ? (
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-4 gap-3 text-center">
            <Stat label="访问次数" value={stats.visits} />
            <Stat label="独立 IP" value={stats.unique_ips} />
            <Stat label="资源访问" value={stats.asset_views} />
            <Stat label="子分享数" value={stats.child_share_count} />
          </div>
          <div>
